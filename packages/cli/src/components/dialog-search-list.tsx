...
export function DialogSearchList<T>({
    ...
    return (
        <box flexDirection="column" gap={1}>
            <input ref={inputRef} placeholder={placeholder} focused onInput={handleContentChange} />
            {filtered.length === 0 ? (
                ...
                <scrollbox ref={scrollRef} height={visibleheight}>
                    {filtered.map((item, i) => {
                        ...
                        return (
                            <box key={getKey(item)} flexDirection="row" height={1} overflow="hidden" backgroundColor={isSelected ? "#89B4FA":undefined} onMouseMove={() => {
                                setSelectedIndex(i)
